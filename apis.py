...
def process_news(news_data, source): # Processes news articles and extracts required fields
    processed_news = []
...
def save_news_to_csv(news, company_name): #Saves news articles to a dedicated folder, ensuring no duplicates and ordering by date."""
    csv_file = os.path.join(OUTPUT_FOLDER, f"{company_name}_news_sentiment.csv")
    existing_titles = read_existing_news(csv_file)

    # Filter out duplicates (DO NOT remove "Unknown" dates for Alpha Vantage)
    ...
    # Sort by date, keeping "Unknown" entries at the bottom
    all_news.sort(key=lambda x: datetime.strptime(x[0], "%d/%m/%Y") if x[0] != "Unknown" else datetime.max)
